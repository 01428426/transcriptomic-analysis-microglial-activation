...
saveRDS(enrich_100,'enrichment_aim1.1.rds')
...
enriched2$cluster[17:24]='2'
enriched2$cluster[25:32]='3'
enriched2$cluster[33:40]='4'
enriched2$cluster[41:48]='5'



names(enrich_threshold)<-c('0','1','2','3','4','5')
...
ggplot(enriched2, aes(cluster,reorder(TermID,enrichment_ratio) , fill = enrichment_ratio, label = star, ylab='genesets')) + 
  ...
  labs(fill = "Enrichment Ratio", label='significance')+
  ylab("genesets")+
  scale_x_discrete(position="top")


write.csv(enrich_100[[4]],'overlap_DAM.csv')
write.csv(enriched2,'enrichment_clusters_aim11.csv')
